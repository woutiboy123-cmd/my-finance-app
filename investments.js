document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (localUid) {
        try {
            const cached = JSON.parse(localStorage.getItem('cache_investments_' + localUid));
    ...
    const CACHE_KEY = 'cache_investments_' + user.id;
    ...
    async function loadData() {
        ...
        const r1 = await sb.from('investment_accounts').select('id, name').order('created_at');
        if (r1.error) { console.error(r1.error); return; }

        const r2 = await sb.from('investment_entries').select('id, account_id, date, balance').order('date');
    ...
    document.getElementById('confirm-modal').onclick = async function () {
        ...
        const { data, error } = await sb.from('investment_accounts')
    ...
    document.getElementById('confirm-entry').onclick = async function () {
        ...
        if (existing) {
            const { error } = await sb.from('investment_entries').update({ balance: balance }).eq('id', existing.id);
            if (error) { console.error(error); return; }
            existing.balance = balance;
        } else {
            const { data, error } = await sb.from('investment_entries')
    ...
    window.deleteEntry = async function (sortedIndex) {
        ...
        const { error } = await sb.from('investment_entries').delete().eq('id', entry.id);
        if (error) { console.error(error); return; }
    ...
    document.getElementById('confirm-delete').onclick = async function () {
        ...
        const { error } = await sb.from('investment_accounts').delete().eq('id', acc.id);
        if (error) { console.error(error); return; }
